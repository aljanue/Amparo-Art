...
const Card = ({ icon, title, desc, linkText, route }) => (
  <div className="bg-white p-10 rounded-3xl shadow-sm hover:shadow-lg transition-all duration-500 group">
    ...
    <button className="text-primary cursor-pointer hover:underline text-xs tracking-widest uppercase font-bold flex items-center gap-2 hover:gap-4 transition-all">
      <Link to={route}>{linkText} <span className="">→</span></Link>
    </button>
  </div>
);
...
function Explore() {
  return (
    <div className="px-8 py-20 bg-background-light flex gap-4 flex-col">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-8">
          ...
          <Card icon="image" title="Portfolio Gallery" desc="Browse the gallery of past commissions. See the details, the style, and the variety of subjects." linkText="View Gallery" route="/portfolio" />
